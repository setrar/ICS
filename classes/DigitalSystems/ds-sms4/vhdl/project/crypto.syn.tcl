...
set_property -dict [list CONFIG.PCW_USE_S_AXI_ACP {1}] $ps7
...
create_bd_port -dir I -type data -from 3 -to 0 btn
create_bd_port -dir O -type data -from 3 -to 0 led
connect_bd_net [get_bd_pins $ip/irq] [get_bd_ports irq]
connect_bd_net [get_bd_pins $ip/sw] [get_bd_ports sw]
connect_bd_net [get_bd_pins $ip/btn] [get_bd_ports btn]
connect_bd_net [get_bd_pins $ip/led] [get_bd_ports led]

# ps7 - ip
apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [list Master "$ps7/M_AXI_GP0" Clk "Auto"] [get_bd_intf_pins $ip/s0_axi]
apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [list Master "$ip/m0_axi" Clk "Auto" ]  [get_bd_intf_pins $ps7/S_AXI_ACP]
...
set_false_path -from [get_ports btn] -to $clock
